...
## 基本步骤
1. **样本采样**：
确定树模型的数量。每棵树的训练集是通过 Bootstrap 抽样所得。即原样本中有 2/3 的数据用于训练树，另 1/3 的数据会作为该树模型的 OOB 样本。
...
前向分步算法将同时求解从 k = 1 ~ K 的所有参数 _β_<sub>_k_</sub> ， _γ_<sub>_k_</sub>  的优化问题简化为逐次求解各个 _β_<sub>_k_</sub> ， _γ_<sub>_k_</sub> 的优化问题（每轮通过最小化损失函数更新模型的相关参数，用于下一轮计算）。
...
### 证明过程
1）![](markdown-img-paste-20200626222556482.png)
上式前半部分（不等式）得证。
...
其中，_J_ 为叶节点个数（表现了单棵树的复杂度），树的参数则为 ![](markdown-img-paste-20200626224037500.png)。树的参数则是我们每轮训练需要求解的。
...
其中 𝜃 为分位数（超参），𝜃 越大，公式前半部分比重越大，表示对预测值低估的惩罚越大，反之对高估惩罚越大。下图中横坐标越正越高估，越负越低估。
