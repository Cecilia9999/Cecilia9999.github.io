...
![](markdown-img-paste-2020062713190430.png)
...
其中，由于分裂成左右两个节点，所以节点数由当前节点总数 _J_ 变成分裂后总数 _J_ + 1。如果增益 > 0，即分裂后，目标函数值下降（损失变小），则按照该特征进行的分裂为有意义分裂，否则该特征直接被 pass。由于增益越大越好，上式经整理后，我们的优化目标为
