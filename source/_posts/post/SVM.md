...
超平面关于训练集的函数间隔 $\hat{\gamma}$ 则为超平面关于训练集中所有点 (x<sup>(i)</sup>，y<sup>(i)</sup>) 的函数间隔 $\hat{\gamma}^{(i)}$ 的最小值
...
已知 x<sub>0</sub> 是超平面上的点，并将上式两边同乘以 w<sup>T</sup>，得

![](markdown-img-paste-20200622142448162.png)

为了使“间隔”是个非负数，对 𝛄 乘上标签 y，则得到超平面关于该点的几何间隔
...
这里的不等式条件是“≥”，而 KKT 条件中不等式符号与之相反，所以这里加入一个负号
...
倒数第三到第二步中 𝞪<sub>_i_</sub> 和 y<sup>(i)</sup> 都是数值，因此转置无影响，倒数第二到最后一步中，使用了乘法运算法则 ``` (a + b + c + … ) (a + b + c + … ) = aa + ab + ac + ba + bb + bc + … ```

2）再求 _L_ 对 𝞪 = { _α_<sub>1</sub> , _α_<sub>2</sub> , ... , _α_<sub>n</sub> } 的极大。具体计算通过 **SMO 算法** 完成。
...
- 对于非支持向量有 ```y*(w*x + b) - 1 > 0```，则当且仅当 _α_<sub>_i_</sub>  = 0 （_α_<sub>i</sub>   ≥  0）时，满足要求。
...
我们先尝试一下上面映射后的内积运算：
假设一般二次多项式情况，我们有一个数据集特征分布在 _d_ 维空间，x = ( x<sub>_1_</sub> , x<sub>_2_</sub> , ... , x<sub>_d_</sub> ) 是其特征维度。按照以下映射，将特征升维
...
给定函数 𝜅，能否使用它来替代计算映射的内积 𝚽 (x<sub>_i_</sub>)・𝚽 (x<sub>_j_</sub>)，或是使定义 𝜅 ( x<sub>_i_</sub> ・x<sub>_j_</sub> ) = 𝚽 (x<sub>_i_</sub>)・𝚽 (x<sub>_j_</sub>) 成立的映射 𝝓 是否存在？

>
> **Mercer定理**：设  $K: ℝ^{n} × ℝ^{n} ↦ ℝ$ （从两个 n 维向量映射到实数域），则 核函数有效  ⇔  对于任何 _X_ = { x<sup>(_1_)</sup> , x<sup>(_2_)</sup> , ... , x<sup>(_m_)</sup> } ，核函数对应的核矩阵必须是对称且半正定的。
>
